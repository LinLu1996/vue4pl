<template>
  <div>
    ...
    <div class="module-wrap">
      <div id="carrierDamage" style="width:100%;height: 18rem;margin: 0 auto"></div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      init () {
        document.getElementById('carrierDamage').removeAttribute('_echarts_instance_');
        this.$http({
          ...
          if (data && data.code === 0) {
            if (this.carrierDamage) {
              this.carrierDamage.clear()
            }
            const datas = data.trEfficiencyPlatformEntity || {}
            const tkpiAssessmentRecordMonthList = datas.fsCargoDamageMonthList || []
      ...
      drawHomePage (dataList) {
        ...
        this.carrierDamage = echarts.init(document.getElementById('carrierDamage'))
        var option = {
          baseOption: {
            timeline: {
              data: indexAll,
        ...
        this.carrierDamage.setOption(option)
        window.addEventListener('resize', () => {
          this.carrierDamage.resize()
        })
        const _this = this
        this.carrierDamage.on('click', function (params) {
