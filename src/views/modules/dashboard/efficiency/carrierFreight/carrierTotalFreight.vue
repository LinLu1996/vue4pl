<template>
  <div>
    <div id="carrierFreight2"  style="width:100%;height: 14rem;margin: 0 auto"></div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      init (val) {
        ...
        document.getElementById('carrierFreight2').removeAttribute('_echarts_instance_');
        this.$http({
          ...
          if (data && data.code === 0) {
            if (this.carrierFreight2) {
              this.carrierFreight2.clear()
            }
            const datas = data.trEfficiencyPlatformEntity || {}
            const tKpiAssessmentRecordYearList = datas.fsCarriageStatementYearList || []
      ...
      drawHomePage2 (datas) {
        ...
        datas.length && datas.forEach(item => {
          data.push({
            ...
            value: item.totalCarriageAmtFaxCount
          })
        })
        var result = [];
        ...
        this.carrierFreight2 = echarts.init(document.getElementById('carrierFreight2'))
        var option = {
          baseOption: {
            timeline: {
              data: indexAll,
        ...
        this.carrierFreight2.setOption(option)
        window.addEventListener('resize', () => {
          this.carrierFreight2.resize()
        })
        var _this = this
        this.carrierFreight2.on('click', function (params) {
            ...
            if (params.data || params.value) {
              _this.$router.push({
                ...
                query: {
                  ...
                  title: `${params.data ? params.data.carrierName : data.length ? data[parseInt(params.value) - 1].carrierName : ''}${new Date(_this.dataForm.rensureYear).getFullYear()}度运费统计`,
                  type: 1
                }
              })
            }
