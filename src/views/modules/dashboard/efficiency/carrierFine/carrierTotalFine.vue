<template>
  <div>
    <div id="myChartCarrier2"  style="width:100%;height: 14rem;margin: 0 auto"></div>
  </div>
</template>
<script>
      ...
      init (val) {
        ...
        this.$http({
          ...
          if (data && data.code === 0) {
            ...
            const tKpiAssessmentRecordYearList = datas.fsFineYearList || []
      ...
      drawHomePage2 (datas) {
        ...
        datas.length && datas.forEach(item => {
          data.push({
            ...
            value: item.totalFineAmountCount
          })
        })
        var result = [];
            ...
            if (params.data || params.value) {
              _this.$router.push({
                ...
                query: {
                  ...
                  title: `${params.data ? params.data.carrierName : data.length ? data[parseInt(params.value) - 1].carrierName : ''}${new Date(_this.dataForm.rensureYear).getFullYear()}度扣罚统计`,
                  type: 2
                }
              })
            }
