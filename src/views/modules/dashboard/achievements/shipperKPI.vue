<template>
  <div>
    ...
    <div class="bg-wrap">
      <p style="padding: 0.625rem;margin:0;font-size: 1.5rem;color: #ffffff;text-align: center">项目KPI统计</p>
      <!--<el-row style="line-height: 1.875rem;font-weight: bold;text-align: center">-->
        <!--<el-col :span="24" style="text-align: center;color: #ffffff;padding-top: 1.25rem">-->
          <!--项目KPI评分排行-->
        <!--</el-col>-->
      <!--</el-row>-->
      <div id="shipperKPI1" style="width:100%;height: 22rem;margin: 0 auto"></div>
    </div>
    <div class="date-wrap">
      <el-date-picker
    ...
    <div class="left_right_footer">
      <!--<el-row style="line-height: 1.875rem;font-weight: bold;text-align: center">-->
        <!--<el-col :span="24" style="height: 2.25rem;line-height: 2.25rem;text-align: center;color: #ffffff">-->
          <!--项目KPI累计评分排行-->
        <!--</el-col>-->
      <!--</el-row>-->
      <div id="shipperKPI2"  style="width:100%;height: 30rem;margin: 0 auto"></div>
    </div>
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      init () {
        document.getElementById('shipperKPI1').removeAttribute('_echarts_instance_');
        document.getElementById('shipperKPI2').removeAttribute('_echarts_instance_');
        this.getMonthList()
        this.getYearList()
      },
      getMonthList () {
        this.$http({
          url: this.$http.adornUrl('/performancemanagement/projectrecord/getProjectKpi'),
          method: 'post',
          data: this.$http.adornData({
            'kpiMonth': this.dataForm.rensureMonth && formatDate(this.dataForm.rensureMonth, 'Month').split('-').join(''),
            'rensureYear': this.dataForm.rensureYear && new Date(this.dataForm.rensureYear).getFullYear()
          })
        }).then(({data}) => {
          if (data && data.code === 0) {
            if (this.shipperKPI1) {
              this.shipperKPI1.clear()
            }
            const datas = data.list || []
            this.drawHomePage(datas)
          } else {
          }
        })
      },
      getYearList () {
        this.$http({
          url: this.$http.adornUrl('/performancemanagement/projectrecord/getProjectKpiByYear'),
          method: 'post',
          data: this.$http.adornData({
            'kpiYear': this.dataForm.rensureYear && new Date(this.dataForm.rensureYear).getFullYear()
          })
        }).then(({data}) => {
          if (data && data.code === 0) {
            if (this.shipperKPI2) {
              this.shipperKPI2.clear()
            }
            const tKpiAssessmentRecordYearList = data.list || []
      ...
      drawHomePage (dataList) {
        ...
        dataList.length && dataList.forEach(item => {
          data.push({
            projectKpiName: item.transportMethod ? `${item.projectKpiName}-${item.transportMethod}` : `${item.projectKpiName}`,
            id: item.id,
            value: item.score
          })
        })
        ...
        result.forEach((item, index) => {
          ...
          item.forEach((item2, index2) => {
            ...
            carrierList.push(item2.projectKpiName)
            data.push({
              projectKpiName: item2.projectKpiName,
              id: item2.id,
              value: item2.value
            })
          })
        ...
        this.shipperKPI1 = echarts.init(document.getElementById('shipperKPI1'))
        var option = {
          baseOption: {
            timeline: {
              data: indexAll,
        ...
        this.shipperKPI1.setOption(option)
        window.addEventListener('resize', () => {
          this.shipperKPI1.resize()
        })
        const _this = this
        this.shipperKPI1.on('click', function (params) {
          if (params.componentType !== 'timeline') {
            if (params.data || params.value) {
              _this.$router.push({
                path: '/dashboard-achievements/achievements/template-view',
                query: {
                  id: (params.data && params.data.id) || data[parseInt(params.value) - 1].id,
        ...
        if (params.componentType !== 'timeline') {
          if (params.data) {
            this.$router.push({
              ...
              query: {
                id: params.data.id,
                rensureMonth: formatDate(this.dataForm.rensureMonth, 'Month'),
                type: 'shipperKpi'
              }
            })
          }
      ...
      drawHomePage2 (datas) {
        ...
        datas.length && datas.forEach(item => {
          data.push({
            projectKpiName: item.transportMethod ? `${item.projectKpiName}-${item.transportMethod}` : `${item.projectKpiName}`,
            projectKpiId: item.projectKpiId,
        ...
        result.forEach((item, index) => {
          ...
          item.forEach((item2, index2) => {
            ...
            data.push({
              projectKpiName: item2.projectKpiName,
              projectKpiId: item2.projectKpiId,
              transportMethod: item2.transportMethod,
              value: item2.value
            })
          })
          options.push({
        ...
        this.shipperKPI2 = echarts.init(document.getElementById('shipperKPI2'))
        var option = {
          baseOption: {
            timeline: {
              data: indexAll,
        ...
        this.shipperKPI2.setOption(option)
        window.addEventListener('resize', () => {
          this.shipperKPI2.resize()
        })
        var _this = this
        this.shipperKPI2.on('click', function (params) {
          if (params.componentType !== 'timeline') {
            if (params.data || params.value) {
              _this.$router.push({
                name: 'achievements-performance-information',
                query: {
                  id: (params.data && params.data.projectKpiId) || data[parseInt(params.value) - 1].projectKpiId,
                  carrierName: (params.data && params.data.projectKpiName) || data[parseInt(params.value) - 1].projectKpiName,
                  // transportMethod: (params.data && params.data.transportMethod) ? params.data.transportMethod : '',
                  transportMethod: (params.data && params.data.transportMethod) || (data.length && data[parseInt(params.value) - 1] && data[parseInt(params.value) - 1].transportMethod),
...
<style lang="scss" scoped>
  .date-wrap{
    ...
    background: url("~@/assets/img/project_date.png") no-repeat;
