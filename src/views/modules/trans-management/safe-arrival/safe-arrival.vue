<template>
  <div>
    <el-form :model="dataForm" @keyup.enter.native="getSearchDataList()" label-width="100px">
      <el-row>
        ...
        <el-col :span="24" style="text-align: right;padding-right: 5px">
          <el-form-item>
            <el-button v-if="isAuth('transmanagement:tmsafearrival:list')" type="warning" icon="el-icon-search" @click="getSearchDataList()">查询</el-button>
            <el-button v-if="isAuth('transmanagement:tmsafearrival:list')" style="background-color: #30b6aa; color: white" icon="el-icon-refresh" @click="resetForm()">重置</el-button>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="24" style="text-align: right;padding-right: 5px">
          <el-form-item style="margin-bottom: 5px">
            <el-button v-if="isAuth('transmanagement:tmsafearrival:trueExport')" type="success" size="mini" icon="el-icon-upload" @click="uploadPageHandle(dataForm.id)">导入</el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    <el-table
      ...
      <el-table-column
        v-if="isAuth('transmanagement:tmsafearrival:info')"
        header-align="center"
        align="center"
        fixed="right"
        width="160"
        ...
        <template slot-scope="scope">
          <el-button v-if="isAuth('transmanagement:tmsafearrival:info')" type="warning" size="small" @click="showDetails(scope.row.id)">
            <icon-svg name="详情" style="vertical-align: middle"></icon-svg>
            详情
          </el-button>
          <el-button v-if="isAuth('transmanagement:tmsafearrival:delete')" type="danger" size="small" icon="el-icon-delete" @click="deleteHandle(scope.row.id)">
            删除
          </el-button>
        </template>
      </el-table-column>
    ...
    <upload-page
      ...
      ref="uploadPage"
      title="赛诺菲安全到货导入"
      fileName="赛诺菲安全到货导入模板.xlsx"
      uploadUrl="/transmanagement/tmsafearrival/export"
      submitUrl="/transmanagement/tmsafearrival/trueExport"
      :acceptType="['xls', 'xlsx']"
...
<script>
  ...
  import SafeDetails from './safe-arrival-details'
  ...
  export default {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/transmanagement/tmsafearrival/list '),
          method: 'post',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定删除这条数据?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/transmanagement/tmsafearrival/delete'),
            method: 'post',
